...
#include <stdint.h>
#include <time.h> // added to allow c tools to compile

int C_int_max =
    INT_MAX; /* KLUDGE TO LET PYREX CODE ACCESS VALUE OF INT_MAX MACRO */
...
/*   } */
/*   if (i!=n) { */
/*     fprintf(stderr,"WARNING: number of records read %d does not match
 * allocation %d\n",i,n); */
/*   } */
/*   return im; */
...
int imstart_qsort_cmp(
    const void *void_a,
    const void *void_b) { /* STRAIGHTFORWARD COMPARISON OF SIGNED start VALUES,
                             LONGER INTERVALS 1ST */
  ...
  if (a->start < b->start)
    ...
    return -1;
           ...
           b->end) /* CONTAINED INTERVAL SHOULD FOLLOW LARGER INTERVAL*/
    return 1;
  else
    return 0;
}
...
int im_qsort_cmp(const void *void_a,
                 const void *void_b) { /* MERGE FORWARD AND REVERSE INTERVALS AS
                                          IF THEY WERE ALL IN FORWARD ORI */
...
int sublist_qsort_cmp(
    const void *void_a,
    const void *void_b) { /* SORT IN SUBLIST ORDER, SECONDARILY BY start */
  IntervalMap *a = (IntervalMap *)void_a, *b = (IntervalMap *)void_b;
...
SublistHeader *build_nested_list_inplace(IntervalMap im[], int n, int *p_n,
                                         int *p_nlists) {
...
#endif
#ifdef MERGE_INTERVAL_ORIENTATIONS
  qsort(im, n, sizeof(IntervalMap),
        im_qsort_cmp); /* SORT BY start, CONTAINMENT */
#else
  qsort(im, n, sizeof(IntervalMap),
        imstart_qsort_cmp); /* SORT BY start, CONTAINMENT */
#endif
  nlists = 1;
  for (i = 1; i < n; ++i) {
  ...
  *p_nlists = nlists - 1;
  ...
  if (nlists == 1) {
    *p_n = n;
    CALLOC(subheader, 1, SublistHeader); /* RETURN A DUMMY ARRAY, SINCE NULL
  ...
  for (i = 1; i < n;) {
    if (isublist &&
        (END_POSITIVE(im[i]) > END_POSITIVE(im[parent]) /* i NOT CONTAINED */
         ||
         (END_POSITIVE(im[i]) == END_POSITIVE(im[parent]) /* SAME INTERVAL! */
          && START_POSITIVE(im[i]) == START_POSITIVE(im[parent])))) {
      subheader[isublist].start = subheader[im[parent].sublist].len -
                                  1; /* RECORD PARENT RELATIVE POSITION */
...
SublistHeader *build_nested_list(IntervalMap im[], int n, int *p_n,
                                 int *p_nlists) {
  ...
  while (i < n) {
    ...
    i = parent + 1;
    while (i < n && parent >= 0) {
      if (END_POSITIVE(im[i]) > END_POSITIVE(im[parent]) ||
          (END_POSITIVE(im[i]) == END_POSITIVE(im[parent]) &&
           START_POSITIVE(im[i]) == START_POSITIVE(im[parent])))
    ...
    if (parent < 0) {
      nlists++; // ADDED THIS IF STATEMENT TO FIX BUG WHERE OVERLAPPING EDGES
                // WOULD NOT CREATE INTERVAL
    }
  } /* AT THIS POINT sublist IS EITHER -1 IF NOT IN SUBLIST, OR INDICATES
       parent*/

  if (nsub > 0) { /* WE HAVE SUBLISTS TO PROCESS */
    CALLOC(imsub, nsub,
           IntervalMap); /* TEMPORARY ARRAY FOR REPACKING SUBLISTS */
    for (i = j = 0; i < n;
         i++) { /* GENERATE LIST FOR SORTING; ASSIGN HEADER INDEXES*/
      parent = im[i].sublist;
      ...
      if (parent >= 0) { /* IN A SUBLIST */
        ...
        j++;
        if (im[parent].sublist <
            0) { /* A NEW PARENT! SET HIS SUBLIST HEADER INDEX */
    ...
    for (i = j = 0; i < n; i++) /* COMPRESS THE LIST TO REMOVE SUBLISTS */
      if (im[i].start != -1 ||
          im[i].end != -1) { /* NOT IN A SUBLIST, SO KEEP */
...
inline int64_t find_overlap_start(int64_t start, int64_t end, IntervalMap im[],
                                  int n) {
  int64_t l = 0, mid, r;
...
inline int64_t find_suboverlap_start(int64_t start, int64_t end, int isub,
                                     IntervalMap im[],
                                     SublistHeader subheader[]) {
  int64_t i;

  if (isub >= 0) {
    i = find_overlap_start(start, end, im + subheader[isub].start,
                           subheader[isub].len);
    if (i >= 0)
...
int64_t find_intervals(IntervalIterator *it0, int64_t start, int64_t end,
                       IntervalMap im[], int n, SublistHeader subheader[],
                       int nlists, IntervalMap buf[], int nbuf, int *p_nreturn,
  ...
  do {
    while (it->i >= 0 && it->i < it->n &&
           HAS_OVERLAP_POSITIVE(im[it->i], start, end)) {
      memcpy(buf + ibuf, im + it->i,
             sizeof(IntervalMap)); /*SAVE THIS HIT TO BUFFER */
      ibuf++;
      k = im[it->i].sublist; /* GET SUBLIST OF i IF ANY */
      it->i++;               /* ADVANCE TO NEXT INTERVAL */
      if (k >= 0 &&
          (j = find_suboverlap_start(start, end, k, im, subheader)) >= 0) {
...
#if defined(ALL_POSITIVE_ORIENTATION) || defined(MERGE_INTERVAL_ORIENTATIONS)
  reorient_intervals(ibuf, buf,
                     ori_sign); /* REORIENT INTERVALS TO MATCH QUERY ORI */
#endif
...
int find_file_start(IntervalIterator *it, int start, int end, int isub,
                    IntervalIndex ii[], int nii, SublistHeader *subheader,
                    int nlists, SubheaderFile *subheader_file, int ntop,
                    int div, FILE *ifile) {
  ...
  if (i_div >= 0) { /* READ A SPECIFIC BLOCK OF SIZE div */
    ...
    it->nii =
        nii + offset_div; /* SAVE INFORMATION FOR READING SUBSEQUENT BLOCKS */
...
int find_file_intervals(IntervalIterator *it0, int start, int end,
                        IntervalIndex ii[], int nii, SublistHeader subheader[],
                        int nlists, SubheaderFile *subheader_file, int ntop,
                        int div, FILE *ifile, IntervalMap buf[], int nbuf,
  ...
  if (it->n == 0) /* DEFAULT: SEARCH THE TOP NESTED LIST */
    if (find_file_start(it, start, end, -1, ii, nii, subheader, nlists,
                        subheader_file, ntop, div,
                        ifile) == FIND_FILE_MALLOC_ERR)
      goto handle_malloc_failure;

  do {                            /* ITERATOR STACK LOOP */
    while (it->i_div < it->nii) { /* BLOCK ITERATION LOOP */
      while (it->i >= 0 &&
             it->i < it->n /* INDIVIDUAL INTERVAL ITERATION LOOP */
             && HAS_OVERLAP_POSITIVE(it->im[it->i], start, end)) { /*OVERLAPS!*/
        memcpy(buf + ibuf, it->im + it->i,
               sizeof(IntervalMap)); /*SAVE THIS HIT */
        ibuf++;
        k = it->im[it->i].sublist; /* GET SUBLIST OF i IF ANY */
        it->i++;                   /* ADVANCE TO NEXT INTERVAL */
        PUSH_ITERATOR_STACK(it, it2, IntervalIterator); /* RECURSE TO SUBLIST */
        if (k >= 0 && (ov = find_file_start(it2, start, end, k, ii, nii,
                                            subheader, nlists, subheader_file,
                                            ntop, div, ifile)) >= 0)
          it = it2; /* PUSH THE ITERATOR STACK */
      ...
      if (it->i == it->n /* USED WHOLE BLOCK, SO THERE MIGHT BE MORE */
        ...
        it->n = read_imdiv(ifile, it->im, div, it->i_div,
                           it->ntop); /*READ NEXT BLOCK*/
...
int repack_subheaders(IntervalMap im[], int n, int div,
  ...
  memcpy(subheader, sub_pack,
         nlists * sizeof(SublistHeader)); /* SAVE REORDERED LIST*/
...
char *write_binary_files(IntervalMap im[], int n, int ntop, int div,
  ...
  npad =
      write_padded_binary(im, ntop, div, ifile); /* WRITE THE TOP LEVEL LIST */
  for (i = 0; i < nlists; i++) {
    ...
    if (subheader[i].len > div) /* BIG LIST: PAD TO EXACT MULTIPLE OF div */
      npad += write_padded_binary(im + subheader[i].start, subheader[i].len,
                                  div, ifile);
    else { /* SMALL LIST: SAVE W/O PADDING */
      fwrite(im + subheader[i].start, sizeof(IntervalMap), subheader[i].len,
             ifile);
  ...
  for (i = 0; i < nlists; i++) /* ALSO STORE INDEX DATA FOR BIG SUBLISTS */
    if (subheader[i].len > div)
      nii += write_binary_index(im + subheader[i].start, subheader[i].len, div,
                                ifile);
  fclose(ifile);
...
/*   if(nlists>0){ */
/*     sprintf(path,"%s%s.subhead",buildpath,filestem); /\* SAVE THE SUBHEADER
 * LIST *\/ */
/*     ifile=fopen(path,"wb"); /\* binary file *\/ */
/*     if (!ifile) */
/*       goto unable_to_open_file; */
...
/* sprintf(path,"%s%s.idb",buildpath,filestem); /\* SAVE THE ACTUAL INTERVAL
 * DB*\/ */
/* ifile=fopen(path,"wb"); /\* binary file *\/ */
/* if (!ifile) */
/*   goto unable_to_open_file; */
...
/*   printf("*p_nlists %d\n", *p_nlists); */
/*   find_intervals(it, 0, 500, im, len, sl, *p_nlists, im_buf,
 * interval_map_size, nhits, &it); */
/*   printf("*nhits %d\n", *nhits); */

/*   int i; */
/*   for (i = 0; i < *nhits; i++){ */
/*     printf("Start %d End %d Id %d\n", im_buf[i].target_start,
 * im_buf[i].target_end, im_buf[i].target_id); */
/*   } */
...
int find_intervals_stack(int start_stack[], int end_stack[], int sp, int start,
  ...
  while (sp >= 0) {
    ...
    while (start_stack[sp] >= 0 && start_stack[sp] < end_stack[sp] &&
      ...
      /* printf("Interval added: %d, %d, %d\n", im[start_stack[sp]].start,
       * im[start_stack[sp]].end, im[start_stack[sp]].target_id); */
      memcpy(buf + nfound, im + start_stack[sp],
             sizeof(IntervalMap)); /*SAVE THIS HIT TO BUFFER */
      ...
      if (k >= 0 &&
        ...
        start_stack[sp] = j;
        end_stack[sp] =
            subheader[k].start + subheader[k].len; /* END OF SUBLIST */
      }
