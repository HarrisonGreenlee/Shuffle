...
extern int64_t find_overlap_start(double start, double end, IntervalMap im[],
                                  int n);
...
extern SublistHeader *build_nested_list(IntervalMap im[], int n, int *p_n,
                                        int *p_nlists);
...
extern double find_intervals(IntervalIterator *it0, double start, double end,
                             IntervalMap im[], int n, SublistHeader subheader[],
                             int nlists, IntervalMap buf[], int nbuf,
                             int *p_nreturn, IntervalIterator **it_return);
extern int read_imdiv(FILE *ifile, IntervalMap imdiv[], int div, int i_div,
                      int ntop);
...
extern int find_file_intervals(IntervalIterator *it0, int start, int end,
                               ...
                               SubheaderFile *subheader_file, int ntop, int div,
                               FILE *ifile, IntervalMap buf[], int nbuf,
                               int *p_nreturn, IntervalIterator **it_return);
...
extern int save_text_file(char filestem[], char err_msg[], char basestem[],
                          FILE *ofile);
extern int text_file_to_binaries(FILE *infile, char buildpath[],
                                 char err_msg[]);
...
#define SET_INTERVAL_POSITIVE(IM, START, END)                                  \
  if ((IM).start >= 0) {                                                       \
    START = (IM).start;                                                        \
    END = (IM).end;                                                            \
  } else {                                                                     \
...
#define HAS_OVERLAP_POSITIVE(IM, START, END)                                   \
  (((IM).start >= 0) ? ((IM).start < (END) && (START) < (IM).end)              \
